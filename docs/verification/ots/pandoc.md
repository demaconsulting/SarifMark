...
The requirements report and trace matrix HTML documents are excluded from the verification
scenarios because they are generated by the compliance toolchain itself (ReqStream and Pandoc
together) — using them as verification evidence for Pandoc would create a self-referential
dependency where the tool being verified is also responsible for producing the evidence.
