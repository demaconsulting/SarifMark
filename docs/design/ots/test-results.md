...
process. SarifMark uses it in the SelfTest subsystem to record the outcomes of its three
...
produces SarifMark — it is an independently developed and separately released package with its own
requirements, versioning, and release lifecycle. It is not part of the SarifMark product and its
internal design requirements do not drive SarifMark requirements. It is therefore classified as an
OTS software item.

### Features Used

- **`TestResults`** — mutable collection that accumulates `TestResult` records during a validation
  run. Passed to `WriteResultsFile` after all three self-validation scenarios have executed.
...
`src/DemaConsulting.SarifMark/DemaConsulting.SarifMark.csproj`. No initialization or
...
pass and passed into each call to `RunValidationTest`. After all three scenarios complete,
